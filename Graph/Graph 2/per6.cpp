...
void djikstra(int data[N][N], int start, int target)
{
    ...
    while (current != (target - 1))
    {
        ...
        for (i = 0; i < 6; i++)
        {
            if (weightCheck[current][i] < check)
            {
                ...
                countBeban += weightCheck[current][i];
            }
        }
    ...
    cout << "Beban = ";
    printDjikstra(beban);
    cout << "Beban Total Terkecil = " << countBeban;
}
...
int main()
{
    ...
    int asal, tujuan;
    int Beban[N], Rute[N] = {0, 0, 0, 0, 0, 0};
    int input[N][N] = {M, 2, M, M, M, M,
                       M, M, 3, M, 7, M,
                       3, M, M, 2, 1, M,
                       M, M, M, M, 1, M,
                       M, M, M, M, M, 2,
    ...
    cout << "Case Study Graph With Djikstra Algorithm" << endl;
    cout << "            Jalur Bis Kota              " << endl;
    cout << "=========================================" << endl;
